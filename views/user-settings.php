...
<?php if ( ! empty( $params['error'] ) ) : ?>
	<div id="message" class="error fade">
		<p><?php esc_html( $params['error'] ); ?></p>
	</div>
<?php endif; ?>

<?php if ( ! empty( $params['message'] ) ) : ?>
	<div id="message" class="updated">
		<p><?php esc_html( $params['message'] ); ?></p>
	</div>
<?php endif; ?>
...
?>
	...
	<form id="new-key-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
		<table class="form-table">
			<tbody>
				<tr>
					<th scope="row"><label for="key-name"><?php esc_html_e( 'Key Name', 'ww-yubiotp-admin' ); ?></label></th>
					<td><input type="text" id="key-name" required="required" name="yotp[name]" value="<?php echo esc_attr( $params['name'] ); ?>" class="regular-text"/></td>
				</tr>
				<tr>
